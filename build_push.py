...
def vers_and_push(
    ...
    new_version = get_versions(
        api_url=api_url, owner=owner, image_name=image_name, token=token) + 1
    new_version = str(new_version)

    build_and_push_image(dockerfile=dockerfile, image_name=image_name,
                         vers=new_version, platforms=platforms, base_image=base_image)
...
class Image(ManageImages):
    ...
    def get_version_number(self):
        url = f"{self.api_url}/users/{self.owner}/packages/container/{self.short_name}/versions"
...
def main():
    ...
    vers_and_push(api_url, owner, install_file, install_name,
                  token, base_image=download_name)
